...
if __name__ == "__main__":
    ...
    df, training_df = load_and_preprocess_data(input_file, target_date)

    # Step 1: Train the model on the available data up till one day of the target date 
    print("Training the bond model...")
    model, X_scaler, y_scaler = model_training.train_sgd_model(training_df, target_date, is_incremental=True)

    # Step 2: Predict bond prices 
    print(f"Predicting bond prices for {target_date}...")
    predictions_df = prediction.predict_for_next_day(df, target_date)

    # Step 3: Compare predictions with actual values for 20
    if predictions_df is not None:
        comparison_df = compare_actual_vs_predicted(df, target_date, predictions_df)

        # Save the comparison results
        output_file = os.path.join('//Users/kanakgarg/Desktop/Kanak/FinanceRL/data/processed/comparison.csv')
